...
if(isset($_POST['submit'])){
  ...
  if((empty($uname)) || (empty($pass))){
    header("Location: ../uac_login.php?login=erroremptyform");
    exit();
  }else{
    ...
    if($resultCheck<1){
      header("Location: ../uac_login.php?login=errornorec");
      exit();
    }else{
      ...
      if(base64_encode($pass) == $row['password']){
        ...
        header("Location: ../uac_home.php?login=success");
        exit();
        //var_dump($_SESSION['logindetails']);

      }
      else{
        header("Location: ../uac_login.php?login=errorpassfail");
        exit();
      }
      }
    }
  ...
  header("Location; ../uac_login.php?login=error");
}
?>
